...
def loggedin(request):
    if not request.user.username == "":
        return render(request,'cbt2/loggedin.html',{"full_name" : request.user.username})
    else:
    ...
    try:
        ...
        if int(num) == 9:
            ...
            for i in range (1,int(num)+1):
                h=request.POST.get('depression_question_%d' %i)
                q1=q1+int(h)
                q2=q2+h
                """
            q2 = (request.POST.get('score'))
            ...
            """
            for i in range (1,int(num)+1):
                h=request.POST.get('anxiety_question_%d' %i)
                q1=q1+int(h)
                q2=q2+h
                """
            q2 = request.POST.get('score');
        ...
        if int(num) == 9:
            return render(request,'cbt2/depression_assessment_questionnaire.html',{'error_message': "select one option for each question",'quiz_list':depression_list},)
        elif int(num) == 7:
            return render(request,'cbt2/anxiety_assesment_questionnaire.html',{'error_message': "select one option for each question",'quiz_list': anxiety_list},)
...
def show_depressionquiz(request):
    ...
    return render(request,'cbt2/depression_assessment_questionnaire.html',{'quiz_list': depression_list})

def show_anxietyquiz(request):
    return render(request,'cbt2/anxiety_assesment_questionnaire.html',{'quiz_list': anxiety_list})